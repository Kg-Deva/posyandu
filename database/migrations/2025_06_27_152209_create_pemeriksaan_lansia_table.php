...
class CreatePemeriksaanLansiaTable extends Migration
{
    public function up()
    {
        Schema::create('pemeriksaan_lansia', function (Blueprint $table) {
            ...
            // PUMA
            $table->tinyInteger('puma_jk');
            $table->tinyInteger('puma_usia');
            $table->tinyInteger('puma_rokok');
            $table->tinyInteger('puma_napas');
            $table->tinyInteger('puma_dahak');
            $table->tinyInteger('puma_batuk');
            $table->tinyInteger('puma_spirometri');
            ...
            $table->foreign('nik')->references('nik')->on('users')->onDelete('cascade'); // ✅ TAMBAH ini
        });
    }
